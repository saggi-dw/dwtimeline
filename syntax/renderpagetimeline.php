...
class syntax_plugin_dwtimeline_renderpagetimeline extends SyntaxPlugin
{
    ...
    public function getSort()  { return 400; }
    ...
    public function render($mode, Doku_Renderer $R, $data)
    {
        ...
        foreach ($headers as $h) {
            if ($h['level'] === 1) { $titleHdr = $h; break; }
        }
        if ($titleHdr === null && !empty($headers)) $titleHdr = $headers[0];
     ...
     */
    private function buildSyntheticTimeline(string $wikitext, array $headers, string $title, int $milLevel, string $align): string
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            $h = $headers[$i];
            if (($h['level'] ?? null) !== $milLevel) continue;
            ...
            for ($j = $i + 1; $j < $count; $j++) {
                ...
                if (($hn['level'] ?? PHP_INT_MAX) <= $milLevel) {
                    ...
                    if ($nextPos >= 0) $end = $this->lineStartAt($wikitext, $nextPos);
                    break;
                }
            }
